...
```spl
index="*" (SourceHostname="WK3.megacorpone.com" OR host="WK3")
| stats count by DestinationIp 
| sort -count 
| head 20
```
...
```spl
index="*" EventCode=11 TargetFilename="*security_update.exe"
| sort _time 
| head 1
...
```spl
index="*" EventCode=1 Image="*\\security_update.exe"
| sort _time 
| head 1
...
```spl
index="*" (User="m.smith" OR Account_Name="m.smith")
